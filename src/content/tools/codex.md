...
Codex CLI 是 OpenAI 的终端 AI 编程助手。它直接运行在你的终端里，能读取代码、执行命令、修改文件，以 Agent 的方式完成编程任务。你可以把它理解成一个会遵守仓库上下文的命令行协作者：给它一个目标，它会拆解步骤、写代码、跑测试、根据错误继续修正。

与 IDE 内嵌补全不同，Codex 的优势在于“进入仓库后完成一段工作”：读文件、改文件、跑命令、解释 diff、做 review。它更适合任务型开发，而不是边写边补全。
...
可以用 `--sandbox` 和 `--ask-for-approval` 控制它能否写文件、能否执行命令、何时需要人工确认。

| 选项 | 用途 |
|------|------|
| `--sandbox read-only` | 只读分析，不允许改文件 |
| `--sandbox workspace-write` | 允许修改当前工作区 |
| `--sandbox danger-full-access` | 无沙箱限制，只适合外部已隔离环境 |
| `--ask-for-approval on-request` | 需要时请求确认 |
| `--ask-for-approval never` | 不请求确认，适合非交互自动化 |
...
```bash
codex --model <strong-model> "复杂任务"
codex --model <fast-model> "简单任务，省钱"
```
...
```bash
codex exec --ask-for-approval never "给所有 .go 文件加上版权头注释"
```

注意：务必在干净的 Git 工作区或临时分支运行，方便回滚。
...
- **沙箱执行**：高权限模式建议在 Docker、VM 或临时工作区中运行，避免误删文件。
- **网络依赖**：需要稳定的网络访问 OpenAI API，国内用户需自备代理。
- **成本控制**：复杂任务可能消耗较多 token，建议用快模型处理简单任务，把强模型留给复杂改动。
